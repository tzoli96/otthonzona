...
function MemberRow({ member, handleRemoveMember, isAdmin, agencyId }) {
  ...
  const handleSendCredit = async (memberId) => {
    try {
      ...
      toast.error("Error sending credits: " + errorMessage);
    }
  };

  return (
    <div className="flex flex-col xl:flex-row border-b border-gray-300 py-2 items-center">
      <div className="w-full xl:w-2/12 pr-2 break-words max-xl:my-2">{member.name}</div>
      <div className="w-full xl:w-3/12 pr-2 break-words max-xl:my-2">{member.email}</div>
      <div className="w-full xl:w-2/12 pr-2 max-xl:my-2">{member.role}</div>

      {isAdmin ? (
        <div
          className="w-full xl:w-5/12 pr-2 flex !flex-row max-sm:!flex-col !items-start sm:!items-center max-xl:my-2"
        >
          <button
        ...
        <div className="w-full xl:w-5/12 pr-2 max-xl:my-2">{member.propertyCount} Ingatlanok</div>
      )}
    </div>
  );
}
