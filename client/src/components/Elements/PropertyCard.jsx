      ...
      <Link to={`/${property.id}`}>
        <div className="bg-white h-full flex flex-col rounded-2xl mt-4 md:mt-0 card-shadow relative overflow-hidden">
          <div className="overflow-hidden round-2xl relative">
            {property.isPromoted && (
                <div
                    className="absolute orange-button flex justify-center items-center right-2 top-2 p-2 !h-8 !w-28 shadow-lg">
                  KIEMELT
                </div>
            )}
            <div
                className="absolute z-40 white-button flex justify-center items-center left-2 bottom-2 p-2 !h-8 !w-28 shadow-lg">
          ...
          >
            <div>
              <div>
                <PropertyDetail classes="flex gap-2" icon={pinIcon} text={`${adTypeCucc?.label} ${propertyCucc?.label}`} />
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-2 mt-2">
                {property.rooms ? <PropertyDetail classes="flex gap-2" icon={roomIcon} text={<>{property.rooms} szoba</>} />: ''}
                {property.area ? <PropertyDetail classes="flex gap-2" icon={arrows} text={<>{property.area}m<sup>2</sup></>} /> : ''}
              </div>
            </div>
